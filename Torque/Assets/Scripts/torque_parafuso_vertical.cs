...
public class torque_parafuso_vertical : MonoBehaviour {
 public float distancia, forca;
    ...
    if ((Input.GetKey(KeyCode.UpArrow)) || (Input.GetKey(KeyCode.RightArrow)))
        transform.Translate(Vector3.right * ((distancia*forca)/400) * Time.deltaTime);

    if ((Input.GetKey(KeyCode.DownArrow)) || (Input.GetKey(KeyCode.LeftArrow)) )
        transform.Translate(-Vector3.right * ((distancia * forca) / 400) * Time.deltaTime);

    if ((Input.GetKey(KeyCode.LeftArrow)) || (Input.GetKey(KeyCode.DownArrow)))
        transform.Rotate(Vector3.right, -(distancia*forca) * Time.deltaTime);
